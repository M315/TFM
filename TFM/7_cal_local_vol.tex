...
\section{Change of variable - log-moneyness}\label{sec:change_variable}
...
\section{Variational forms and discretization}\label{sec:variational_forms}
...
Examples 1 and 2 use synthetic data and a time independent coefficient $a^\dagger$.
We now try to calibrate a full surface $a(y,\tau)$ from real market quotes, of the S\&P 500 index,
as we did with implied volatility.
This goes past the results of Section \ref{sec:convergence_rates},
where the rate theory assumes a time independent coefficient,
here the unknown is genuinely time dependent,
so the surface fit is a numerical extension rather than a covered case,
we have not proved convergence rates for this setting.

Like in the implied volatility calibration of Section \ref{sec:impl_vol_surface},
the data are SPY European option quotes from 30 April 2026,
with spot $S_0 = 720.65$.
We use eleven expirations,
with maturities $\tau$ from $0.10$ to $0.88$ years.
For each expiration the risk free rate,
the dividend yield and the implied volatility come from the pipeline of Appendix \ref{app:impl_vol},
the Treasury curve for $r$,
put call parity for $q$,
and the out of the money composite for the smile.
Each smile is restricted to the moneyness band $0.8 \le K/S_0 \le 1.2$,
where the quotes are more liquid.
We keep only expirations with at least twenty five strikes in this band,
a sparser expiration is under-determined.
We also drop two expirations that fall within two weeks of a kept one,
30 August and 31 December 2026,
whose slice would duplicate its neighbour rather than add information.

The surface is held as one spatial slice per maturity,
which is the parametrization \eqref{eq:coefficient_slices} of Section \ref{sec:time_dependent_a},
we do not fit the maturities one at a time nor interpolate between their smiles.
...
Figures~\ref{fig:egger_surface} to~\ref{fig:egger_reprice} show the result for
$\beta_y = 0.1$ and $\beta_\tau = 1$. The
recovered surface has the shape one expects: a pronounced negative skew, with local volatility
running from about $0.15$ at the money at the shortest maturity to $0.23$ at the longest, near
$0.12$ on the upside, and rising sharply on the downside, to about $0.40$ at the long
maturities and much steeper at the short end, close to $0.95$ two tenths of log-moneyness below
the money, where the skew is strongest. Repricing the quotes at the calibrated surface gives a median relative error of
$0.13\%$ across the eleven maturities, with the maximum over liquid strikes staying below $1\%$ at
every expiration. This is markedly tighter than the naive prior $a^* = \tfrac12\sigma_{\mathrm{iv}}^2$,
which gives a median of $0.20\%$ and maxima above $2\%$: that prior anchors the skew at half its
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\textwidth]{egger_example4_surface_3d}
    \caption{Local volatility surface $\sigma(y,\tau)$ calibrated from SPY quotes on 30 April
             2026 ($S_0 = 720.65$, eleven maturities), with $\beta_y = 0.1$ and $\beta_\tau = 1$.}
    \label{fig:egger_surface}
\end{figure}
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\textwidth]{egger_example4_surface_slices}
    \caption{Recovered local volatility slices (solid) of the surface of
             Figure~\ref{fig:egger_surface} against each maturity's market implied volatility
             (faint); the local volatility skew is steeper than the implied one.}
    \label{fig:egger_slices}
\end{figure}
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\textwidth]{egger_example4_surface_reprice}
    \caption{Per maturity relative reprice error of the calibrated surface of
             Figure~\ref{fig:egger_surface} over liquid strikes (market price at least $0.5\%$
             of $S_0$), median and maximum.}
    \label{fig:egger_reprice}
\end{figure}

\section{The local volatility skew and the factor of two}\label{sec:factor_two}

In Figure \ref{fig:egger_slices} we can observe that local volatility has an steeper skew than the implied one.
This is not a consequence of the calibration,
it is the rule of two shown in Section \ref{sec:rule_of_two}.

Given the relationship between local volatility and implied volatility,
from \eqref{eq:factor_two},
near ATM the local volatility skew is twice the implied volatility skew,
we can rewrite it as
\begin{equation}\label{eq:factor_two_derivative}
    \left.\partial_y \sigma_{\mathrm{loc}}\right|_{y=0}
      \;=\; 2\,\left.\partial_y \sigma_{\mathrm{iv}}\right|_{y=0},
\end{equation}
see \cite{bergomi_stochastic_vol} Chapter 2.

To verify such relationship numerically,
we will check \eqref{eq:factor_two_derivative} directly on the calibrated surfaces.
For each maturity $\tau$,
we fit a quadratic to the local volatility and to the smile in a narrow window $|y| \le 0.05$ around the ATM and read the slope at $y = 0$.
In figure \ref{fig:egger_skew} we can see two of the eleven maturities,
drawing from the local volatility level a line with twice the implied slope lands on the local tangent,
which is \eqref{eq:factor_two_derivative} read off the picture.
Table \ref{tab:egger_skew} displays the two slopes at every maturity.
The ratio averages $2.16$ below three months,
and stays between $2.0$ and $2.5$ over the whole term structure.
\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{egger_example5_skew}
    \caption{The rule of two on the SPY surface of Figure~\ref{fig:egger_surface}, at the
             shortest maturity (left) and at three months (right). Market implied volatility
             (dots) and recovered local volatility (solid) near the money, with the tangents at
             $y = 0$ of the quadratic fits over the shaded window $|y| \le 0.05$. The dotted
             line has twice the implied slope and sits on the local tangent.}
...
\begin{table}[h!]
    ...
    \begin{tabular}{c c c c c}
        \toprule
        expiration & $\tau$ & $\partial_y\sigma_{\mathrm{iv}}$
                   & $\partial_y\sigma_{\mathrm{loc}}$ & ratio \\
        \midrule
        2026-06-05 & $0.096$ & $-0.827$ & $-1.659$ & $2.01$ \\
        2026-06-18 & $0.132$ & $-0.718$ & $-1.558$ & $2.17$ \\
        2026-06-30 & $0.164$ & $-0.676$ & $-1.364$ & $2.02$ \\
        2026-07-17 & $0.211$ & $-0.637$ & $-1.504$ & $2.36$ \\
        2026-07-31 & $0.249$ & $-0.611$ & $-1.368$ & $2.24$ \\
        2026-08-21 & $0.307$ & $-0.572$ & $-1.328$ & $2.32$ \\
        2026-09-18 & $0.384$ & $-0.518$ & $-1.138$ & $2.19$ \\
        2026-09-30 & $0.416$ & $-0.522$ & $-1.171$ & $2.24$ \\
        2026-12-18 & $0.633$ & $-0.452$ & $-1.114$ & $2.47$ \\
        2027-01-15 & $0.710$ & $-0.423$ & $-0.919$ & $2.17$ \\
        2027-03-19 & $0.882$ & $-0.390$ & $-0.927$ & $2.38$ \\
        \bottomrule
    \end{tabular}
    \caption{
        At the money skews of the calibrated surface of Figure~\ref{fig:egger_surface}.
        Both slopes are the linear coefficient of a quadratic fit in $|y| \le 0.05$, one on the
        market smile and one on the recovered local volatility slice, and the last column is
        their ratio, the factor of two of~\eqref{eq:factor_two}.
    }
    \label{tab:egger_skew}
\end{table}

\section{Out of sample test: pricing between quoted maturities}\label{sec:holdout}

Both the calibrated local volatility surface and the interpolated implied volatility surface,
agree on the price of quoted maturities by construction.
But they differ on contracts that falls between them.

To test how each method prices out of sample,
we entirely remove one interior expiration from the data,
the December 2026 quotes at $\tau = 0.63$,
and price it out of sample two ways.
The local volatility surface is calibrated on the remaining ten maturities and the Dupire equation is used to recover the prices across the gap,
so the price at $\tau = 0.63$ is a genuine sample of the forward solution rather than an interpolation of the smile.
On the other hand,
the implied volatility is computed at the remaining data points,
then we interpolate the total variance $\sigma_{\mathrm{iv}}^2\tau$ linearly in maturity at fixed log-moneyness between the two closest expirations,
$\tau = 0.42$ and $\tau = 0.71$,
to finally calculate the price from Black-Scholes formula.

Figure \ref{fig:egger_holdout} shows the outcome over the well quoted band $|y| \leq 0.15$.
The deep ITM and OTM strikes are left out of the picture,
deep ITM options are dominated by the intrinsic value $S_0e^{-q\tau} - Ke^{-r\tau}$,
so they carry no volatility information,
deep OTM are less liquid and the relative miss-price is higher due to low original price,
the more far from the money an option is th cheaper it is.
Both methods reproduce the excluded market prices closely,
and neither is ever off by more than $2.0\%$ over the band.
Over the liquid strikes the median relative price error is $0.29\%$ for the implied volatility interpolation
and $0.75\%$ for the local volatility surface.
The interpolation is therefore the more accurate of the two,
this is not a surprise.
Interpolating the total variance is built to reproduce the smile,
it uses the two nearest expirations to interpolate and the market volatility is exact at both of them,
whereas the local volatility surface is a function of the \emph{coefficient} of a partial differential equation,
and it is propagated forward between the observed market points,
so an error in $a(y,\tau)$ accumulates in the solution before it is read off as a price.

Even though the implied volatility looks better on this numerical experiment,
we should not conclude that is a better model,
at the end of the day local volatility model is constructed under the not arbitrage premise,
and market quotes are not always arbitrage free,
thus reproducing the market prices is not necessarily a sign of a better model.
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{egger_example6_holdout}
    \caption{
        Out-of-sample pricing of a held-out expiration (December 2026, $\tau = 0.63$),
        removed from the calibration and bracketed by $\tau = 0.42$ and $\tau = 0.71$.
        Relative price error $|C - C_{\text{mkt}}| / C_{\text{mkt}}$ over the well quoted
        band $|y| \leq 0.15$, for the local volatility surface and for the implied volatility
        interpolation. Neither exceeds $2.5\%$, and the interpolation is the more accurate
        across the band.
    }
