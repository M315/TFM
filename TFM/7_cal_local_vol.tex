...
\section{Calibration of Local Volatility}
...
\subsection{Ill-posedness of the inverse pricing problem}
...
\subsection{Change of variable - log-moneyness}
...
\subsection{Parameter-to-solution map}
...
\subsection{Tikhonov functional}
...
\subsection{Gradient computation via the adjoint method}
...
\end{equation}
\subsection{Convergence rates and the source condition}\label{sec:convergence_rates}
...
\subsection{Choice of $\beta$ and stopping criterion}\label{sec:beta_choice}
...
\subsection{Variational forms and discretization}
...
\subsubsection{A time-dependent coefficient}
...
\subsection{Numerical experiment - Example 1}\label{sec:egger_ex1}
...
\subsection{Numerical experiment - a local volatility surface from market data}\label{sec:market_surface}
...
functional is the space and time version of the Tikhonov
functional~\eqref{eq:tikhonov_functional} announced there,
\begin{equation}\label{eq:surface_functional}
    J(a) = \sum_{j} \bigl\| u(a)(\cdot,\tau_j) - u^\delta_j \bigr\|^2_{L_2(B_j)}
         + \beta_y \sum_{k} \| a_k - a^*_k \|^2_{H^1}
         + \beta_\tau \sum_{k} \| a_k - a_{k-1} \|^2_{L_2},
\end{equation}
where $B_j$ is the observed band at maturity $\tau_j$, the $H^1$ term smooths each slice in
strike and the $L_2$ term penalizes the jump between consecutive slices. The triangular
...
route of Section~\ref{sec:impl_vol}, where a fitted surface with the slightest butterfly or
...
\subsection{The local volatility skew and the factor of two}\label{sec:factor_two}
...
\subsection{Alternative approach: Interpolation/Extrapolation of the IV surface}\label{sec:impl_vol}
...
in the liquid region and extrapolating into the wings, then convert it to a continuous call
price surface $C(K,T)$ through the Black-Scholes formula, and read off the local volatility
...
We build such a surface from market data and use it as the input to the Dupire formula. The
full construction, the risk free curve, the dividend yield calibration, and the out of the
money composite from which the smiles are assembled, is detailed in
Appendix~\ref{app:impl_vol}.

\subsection{Out of sample test: pricing between quoted maturities}\label{sec:holdout}
