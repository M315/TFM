...
\begin{definition}[Probability space]
    ...
    \begin{itemize}
        ...
        \item $\mathbb{P}$ is a probability measure.
    \end{itemize}
\end{definition}
...
\begin{definition}[Stochastic processes]
    ...
    \begin{equation*}
        \{ X(t, \omega) \}_{t \in T},
    \end{equation*}
    on a probability space $(\Omega, \F, \mathbb{P})$ assuming values in $\R^n$.
\end{definition}

If the set $T$ is countable then the stochastic process is said to be \textit{discrete},
if $T$ is some interval of $\R$ then it is \textit{continuous}.

An intuitive way to understand stochastic processes is to consider what is $X(t, \omega)$ when $t$ or $\omega$ is fixed.
...
The idea is that a filtration $\F_t$ represents the information available from the past of an stochastic process up to time $t$.
Thus we define a \textbf{filtered probability space} as a probability space with a filtration $(\Omega, \F, \{ \F_t \}_{t \in T}, \mathbb{P})$.

\begin{definition}[Adapted process]
    Let $X_t$ be a stochastic process on a filtered probability space $(\Omega, \F, \{ \F_t \}_{t \in T}, \mathbb{P})$.
...
as it ensures that the process at time $t$ depends only on the information available up to that time,
but not on future information.

\begin{definition}[Quadratic variation \cite{steele_stochastic} Ch. 8.6]
    Let $X_t$ be a stochastic process on a filtered probability space $(\Omega, \F, \{ \F_t \}_{t \in T}, \mathbb{P})$.
    For any partition $\pi = \{ t_0, t_1, \ldots, t_n \} \subset [0, t]$,
    the $\pi$-quadratic variation of $X_t$ is defined as the random variable
    \begin{equation*}
        Q_\pi(X_t) = \sum_{i = 0}^{n - 1} (X_{t_{i + 1}} - X_{t_i})^2.
    \end{equation*}
    If there is a process $V_t$ such that $Q_\pi(X_t) \rightarrow V_t$ in probability,
    for any sequence of partitions $\pi$ of $[0, t]$,
    with $\mu(\pi) \rightarrow 0$,
    then $V_t$ is called the \textbf{quadratic variation} of $X_t$ and is denoted by $\langle X \rangle_t$.
\end{definition}
...
\begin{definition}[Martingale]
    ...
    with respect to a filtration $\{\F_t\}_{t \in T}$ if it satisfies the following conditions:
    \begin{enumerate}
...
\begin{enumerate}
    ...
    \item $W_t$ has independent increments,
        i.e. for $t_1 < t_2 < t_3 < t_4$, $W_{t_2} - W_{t_1}$ and $W_{t_4} - W_{t_3}$ are independent random variables
...
Also it can be shown that the Wiener process is an almost surely continuous martingale
with quadratic variation $[W, W]_t = t$.
These two properties together with $W_0 = 0$ form an equivalent definition of the Wiener process,
known as Lévy's characterization.

Proofs of these properties, the equivalence between the two definitions,
and the existence of Brownian motion can be found in \cite{morters_peres_brownian_motion}
and \cite[Ch.~2]{oksendal_sde}.
