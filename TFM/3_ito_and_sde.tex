...
Most contents from this chapter are taken from \cite{oksendal_sde} chapters 3 to 5 and \cite{steele_stochastic}.
...
\begin{definition}\label{def:ito_integral_reqs}
    ...
    such that
    \begin{itemize}
        \item $f(t, \omega)$ is $\mathcal{B} \times \mathcal{F}$-measurable, where $\mathcal{B}$ is the Borel $\sigma$-algebra on $[0, \infty)$ and $\mathcal{F}$ is the $\sigma$-algebra on $\Omega$,
...
\begin{equation*}
    \int^{T}_{S} \phi(t, \omega)\, dW_t(\omega) = \sum_{j} e_j(\omega) \left[ W_{t_{j + 1}} - W_{t_{j}} \right] (\omega).
\end{equation*}

\begin{lemma}[Ito isometry]
    Let $\phi(t, \omega)$ be a bounded elementary function, then
    \begin{equation*}
        \E \left[ \left( \int^{T}_{S} \phi(t, \omega) dW_t (\omega) \right)^2 \right]
...
\begin{equation*}
    \mathbb{P} \left( \int^{T}_{S} f(t, \omega)^2 dt < \infty \right) = 1,
\end{equation*}
...
\begin{definition}[Ito process]
    Let $W_t$ be a Brownian motion on $(\Omega, \F, \mathbb{P})$.
    An Ito process is a stochastic process $X_t$ on $(\Omega, \F, \mathbb{P})$ of the form
    \begin{equation}\label{eq:ito_process}
        X_t = X_0 + \int^{t}_{0} \mu(s, X_s) ds + \int^{t}_{0} \sigma(s, X_s) dW_s,
    \end{equation}
    where $\mu$ and $\sigma$ are adapted, measurable processes, that satisfy the integrability conditions
    \begin{equation*}
        \mathbb{P} \left( \int^{T}_{0} \left| \mu(t, \omega) \right| dt < \infty \right) = 1,
    \end{equation*}
    and
    \begin{equation*}
        \mathbb{P} \left( \int^{T}_{0} \sigma(t, \omega)^2 dt < \infty \right) = 1.
    \end{equation*}
\end{definition}
...
For the proof of the Ito's formula we refer to \cite{oksendal_sde} Theorem 4.1.2.
The formula can be extended to multiple dimensions.

\begin{definition}[$n$-dimensional Ito process]
    Let $W_t = (W_1(t), \dots, W_m(t))$ be a $m$-dimensional Brownian motion,
    let $\mu_{i}$ and $\sigma_{ij}$ be adapted, measurable processes such that
    \begin{equation*}
        \mathbb{P} \left( \int^{T}_{0} \left| \mu_{i}(t, \omega) \right| dt < \infty \right) = 1,
        \qquad
        \mathbb{P} \left( \int^{T}_{0} \sigma_{ij}(t, \omega)^2 dt < \infty \right) = 1,
    \end{equation*}
    then we define the $n$-dimensional Ito process $X_t = (X_1(t), \dots, X_n(t))$ as
    \begin{equation*}
        \left\{
        \begin{aligned}
            & dX_1(t) = \mu_1 dt + \sigma_{11} dW_1(t) + \dots + \sigma_{1m} dW_m(t), \\
            & \vdots \\
            & dX_n(t) = \mu_n dt + \sigma_{n1} dW_1(t) + \dots + \sigma_{nm} dW_m(t), \\
        \end{aligned}
        \right.
    \end{equation*}
...
\begin{theorem}[General Itô's formula]\label{thm:ndim_ito_formula}
    ...
    \begin{equation*}
        Y_t = g(t, X_t),
    \end{equation*}
    is a $p$-dimensional Ito process whose components, $0 \leq k \leq p$, are given by
    \begin{equation}\label{eq:ndim_ito_formula}
        dY_k(t) = 
            \frac{\partial g_k}{\partial t}(t, X_t) dt
            + \sum_{0 \leq i \leq n} \frac{\partial g_k}{\partial x_i}(t, X_t) dX_i(t)
            + \frac{1}{2} \sum_{0 \leq i,j \leq n} \frac{\partial^2 g_k}{\partial x_i \partial x_j}(t, X_t) dX_i(t) dX_j(t),
    \end{equation}
...
\subsection{Example: Geometric Brownian motion}
...
\end{equation*}
Integrating both sides from $0$ to $t$ gives $\ln(S_t / S_0) = (\mu - \sigma^2/2)\,t + \sigma W_t$,
so the unique solution is
\begin{equation}\label{eq:gbm_solution}
    S_t = S_0 \exp\!\left[\left(\mu - \frac{\sigma^2}{2}\right) t + \sigma W_t\right].
\end{equation}

\subsection{Example: Ornstein-Uhlenbeck process}
...
\begin{equation*}
    dY_t
        ...
        = e^{\theta t}\bigl[\theta(\mu - X_t) + \theta(X_t - \mu)\bigr] dt + \sigma e^{\theta t}\, dW_t
        = \sigma e^{\theta t}\, dW_t.
\end{equation*}
